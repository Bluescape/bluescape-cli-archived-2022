    ...
    for await (const [index, mappedEmail] of mappedEmails.entries()) {
        ...
        if (ssoEmail) {
            ...
            if ((0, validators_1.valueExists)(targetMember) && targetMember?.id) {
                ...
                spinner.info(chalk_1.default.gray(`${progressing} - ${ssoEmail} is already exist. We will raise request to transfer resources from ${existingEmail} to ${ssoEmail}\n`));
            }
            else {
                if (!sourceMemberBelongsToManyOrgs) {
                    const reportMessage = [];
                    if (sourceMember.role.type === role_types_1.Roles.Visitor) {
                        reportMessage.push(`${existingEmail} role will be updated to ${organization?.defaultOrganizationUserRole?.type}`);
