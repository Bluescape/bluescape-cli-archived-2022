...
const handler = async (argv) => {
    ...
    const orgOwner = await services_1.emailMigrationService.validateOrganizationOwnerExistence(organizationId, existingEmails);
    if (orgOwner && orgOwner.error) {
        spinner.fail(chalk_1.default.red(`Error: ${orgOwner.error}`));
        return;
    }
    if (!orgOwner) {
        spinner.fail(chalk_1.default.red(`No Owner Email information is found in the Existing Email of uploaded Mapping CSV. Please include owner details to map. We cannot proceed without owner mapping.`));
        return;
    }
